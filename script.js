...
function showToDo(array) {
  ...
  for (let todo of array) {
    ...
    doItem.classList.add("list-item");
...
function removeToDoDone() {
  let arrayDelete = arrayToDo.filter((element) => {
    return element.done === true;
  });
  ...
  let arrayRest = arrayToDo.filter((element) => {
    return element.done === false;
  });
  arrayToDo = arrayRest;
  if (arrayToDo.length < 1) {
    doList.innerText = "All things done 😎. \n Chill now and have a nice day!";
